...
if (isset($_POST["search_productName"])) {
    ...
    if ($result->num_rows > 0) {
        ...
        header("location: ../adminprodmng.php?modproduct={$row["product_id"]}");
    } else {
        setPopup("[ ERROR ] No se encontró el producto que buscaste.", "../adminprodmng.php?searchproduct");
        die();
    }
} else if (isset($_POST["search_productList"])) {
    $product_nameSEARCHID = $_POST["search_listName"];

    $sql = "SELECT * FROM productdata WHERE product_id='$product_nameSEARCHID'";
    $result = $conn->query($sql);
    if ($result->num_rows == 1) {
        $row = $row = $result->fetch_assoc();
        header("location: ../adminprodmng.php?modproduct={$row["product_id"]}");
    } else {
        setPopup("[ ERROR ] No se encontró el producto de la lista.<br>Query: $product_nameSEARCHID", "../adminprodmng.php?searchproduct");
        die();
    }
    ...
    if ($conn->query($sql) === TRUE) {
        setPopup("Se actualizó $product_name exitosamente.", "../adminprodmng.php?searchproduct");
    } else {
    ...
    if ($proceed == true) {
        ...
        if ($conn->query($sql) === TRUE) {
            setPopup("Se eliminó $product_name del catálogo.", "../adminprodmng.php?searchproduct");
        } else {
            echo "Error: " . $sql . "<br>" . $conn->error;
        }
    }
    ...
    if ($result->num_rows > 0) {
        ...
        if (isset($_FILES["product_imageChange"]) && $_FILES["product_imageChange"]["error"] == 0) {
            ...
            if (!array_key_exists($ext, $allowed)) {
                setPopup("Ese formato de imagen no se admite en nuestro sitio.", "../adminprodmng.php?modproduct={$_SESSION["ADMIN_curModifProd"]}");
                exit();
            }
            ...
            if (in_array($filetype, $allowed)) {
                ...
                if (file_exists("files/products/" . $filename)) {
                    ...
                    if ($conn->query($sql) === TRUE) {
                        ...
                        setPopup("Se subió la imagen exitosamente.", "../adminprodmng.php?modproduct={$_SESSION["ADMIN_curModifProd"]}");
                    } else {
                        echo "Error: " . $sql . "<br>" . $conn->error;
                    }
                }
