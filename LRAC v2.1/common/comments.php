    ...
    if ($user) {
        echo <<<HTML
            ...
            <form action="conns/createpost.php?category=$category&query=$query" method='post' class='main-maxw main-rowstart main-aligncenter'>
                ...
                <div>
                    <input type="text" id='comm-input' placeholder='Escribe un comentario...' oninput="adjustWidth(this)" name="content" autocomplete="off" required>
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        if (isset($_SESSION["loggedin"]) && $_SESSION["loggedin"] == true) {
            if ($_SESSION["data_isAdmin"] == true) {
                $adminDelete = "
                <a href='conns/deletepost.php?post=$postid&category=$page&query=$query&isAdmin' class='icontext'>
                    <script>
                        icon('2em', '2em', 'eraser')
                    </script>
                    <p>Administrador: Eliminar comentario</p>
                </a>
            ";
            }
        }
        ...
        if ($page == "usercomments") {
            switch ($postcategory) {
                ...
                case 'userprofile':
                    ...
                    break;
                case 'threadpost';
                    $urlUC = "threadpost.php?postid=$postwhere";
                    break;
            }
