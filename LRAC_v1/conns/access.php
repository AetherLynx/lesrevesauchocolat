...
if (isset($_POST["login"])) {
    ...
    $sql = "SELECT name FROM userdata WHERE name = '$name'";
    $result = $conn->query($sql);

    if ($result->num_rows == 0) {
        setPopup("No se encontró una cuenta con ese nombre de usuario.", "../login.php");
        die();
    }

    $sql = "SELECT name, pass FROM userdata WHERE name = '$name' AND pass = '$pass'";
    $result = $conn->query($sql);
